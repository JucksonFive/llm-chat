...
import { registerApprovalRoutes } from './lib/approval-routes.js'
import { streamBedrock } from './bedrock-service.js'
import { logSecurityEvent } from './lib/audit-log.js'
import { wslDriver, windowsDriver, registerSandboxDriver, getDriverStatus } from './sandbox/index.js'
...
// Tool approval routes (session-scoped, in-memory)
...
// ── Workspace folder token verification ──────────────────────────────
// The Electron main process signs folder paths with a short-lived token.
// The renderer must present this token when setting a project workspace.
// In dev mode (no Electron), the check is bypassed for convenience.

function verifyWorkspaceToken(token: string, path: string): boolean {
  const secret = process.env.ELECTRON_IPC_SECRET
  if (!secret) {
    // Dev mode: trust the provided path (localhost-only)
    return true
  }
  // Tokens are UUIDs generated by the Electron main process. The server
  // cannot verify them independently because the token→path mapping lives
  // only in the main process. In production, the server runs inside the
  // same Electron process, so we validate that a secret was set (meaning
  // the main process initialized us) and trust the pre-verified token.
  // The main process only issues tokens after a native dialog selection.
  return typeof token === 'string' && token.length >= 36 && typeof path === 'string' && path.length > 0
}

app.post('/api/verify-folder-ticket', (req, res) => {
  const { ticket, path: folderPath } = req.body ?? {}
  if (!ticket || !folderPath) {
    res.status(400).json({ valid: false, error: 'ticket and path are required' })
    return
  }
  const valid = verifyWorkspaceToken(ticket as string, folderPath as string)
  res.json({ valid, path: folderPath })
})
...
app.get('/api/sandbox/status', async (_req, res) => {
  try {
    const drivers = await getDriverStatus()
    res.json({ drivers })
  } catch (error) {
    res.status(500).json({
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    const { agentId, providerId, model, messages, systemPrompt, mcpServers, builtInToolIds, projectId, permissionProfile } = req.body
    ...
    const projectContext = projectId ? { projectId, permissionProfile } : undefined
...
export async function startServer(port = 3001): Promise<number> {
  ...
  // Register sandbox drivers
  registerSandboxDriver('wsl-pwsh', wslDriver)
  registerSandboxDriver('windows-powershell', windowsDriver)
  console.log('[sandbox] Drivers registered: wsl-pwsh, windows-powershell')
